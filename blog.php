...
<div class="container">
    <div class="row align-items-center blog-items">
        <?php
        /*
         * This script here allows for the portrayal of the different blogs in the website.
         * The page uses bootstrap cards to show the data which are taken by PDOs and a SQL Statement.
         * The SQL Statement gets all the blogs in descending order so that the newest come out first.
         * A foreach loop is then made to bring out the different details for the blog to fill the card,
         * said descriptions are saved in the array "$result" which fetches all results from the statement.
         */
        $stmt = $con->prepare("SELECT * FROM BlogKommentare ORDER BY BlogID DESC");
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $blogCount = 0;
        foreach ($result as $product) {
            ?>
            <div class="col-lg-6 col-md-12 col-sm-12 py-4 blog-item" style="min-width: 200px; height: auto; <?php echo ($blogCount >= 2) ? 'display:none;' : ''; ?>">
                <div class="card mx-auto bg-secondary text-white" style="width:75%; height:100%">
                    ...
                    <div class="card-body text-center" style="height:100%;">
                        <h5 class="card-title fs-6"> <b><?php echo $product["Title"]; ?></b> </h5>
                        <div class="card-title"> <?php echo $product["BlogDate"]; ?></div>
                    </div>
                    <div class="d-flex justify-content-center align-items-center pb-3">
                        <a href="blogDetails.php?id=<?php echo $product["BlogID"]; ?>" class="btn btn-dark text-light">Read
                            More</a>
                    </div>
                </div>
                <div class="p-2"></div>
    ...
    <div class="row d-flex justify-content-center">
        <button id="readMoreBtn" class="btn btn-secondary col-lg-3">Read more blogs</button>
...
<script>
    /*
    * This script toggles visibility of 'blog-item' elements on the page. The 'Read More' button
